...
- Урок 4 — интеграция нейробота в training lab и вывод результатов

Не начато:
- Урок 5 и далее

Точка продолжения:
- **начать с урока 5** (диагностика inference: observation size, action scores и выбранное действие).
...
### Урок 4 — Интеграция в training lab
Файлы:
- `src/ai/nn/trainingLabPolicies.ts`
- `src/app/SnakeGameApplication.ts`
- `tests/training-lab-policies.test.ts`

Результат:
- в dev-лаборатории можно выбрать `random-turns` или `neural-simple-v1`;
- нейросеть получает размеры входа из runtime-настроек Arena;
- случайные веса воспроизводятся по seed;
- результат показывает выбранную политику, счёт, время жизни и причину смерти.
...
3. Начать следующий урок: **урок 5**.
